...
contract Vesting is VestingWallet{
    ...
    function addVest(IERC20 _token, uint256 _amount, uint256 _startTimestamp, uint256 _durationWeeks) external virtual
    {
        require(_startTimestamp > MIN_TIMESTAMP, "Vesting start before minimal date");
    ...
    function vestedAmountUntilWeek(uint256 _endWeek, address _token) internal view returns (uint256)
    {
        uint256 totalVested = 0;
        for(uint i = 0; i < vests.length; i++)
        {
            if(address(vests[i].token) != _token)
                continue;
            else if(vests[i].startWeek > _endWeek)
                continue;
            else if(vests[i].startWeek + vests[i].durationWeeks > _endWeek)
                totalVested += (vests[i].amount*(_endWeek - vests[i].startWeek))/vests[i].durationWeeks;
