...
contract VestingFactory {
  ...
  event CreatedVestingForTeamMember(
    address _teamMemberAddr,
    address _contractAddr
  );

  function createVestingForUser(address _user) external returns (address) {
    require(
      vestingForUser[_user] == address(0),
  ...
  function createVestingForTeamMember(address _teamMember)
    external
    returns (address)
  {
    require(
      vestingForTeamMember[_teamMember] == address(0),
      "Vesting contract already exists"
    );
