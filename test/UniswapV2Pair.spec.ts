...
describe('UniswapV2Pair', () => {
  ...
  it('mint', async () => {
    ...
    await token0.transfer(pair.address, token0Amount).then((res: any) => res.wait());
    await token1.transfer(pair.address, token1Amount).then((res: any) => res.wait());
  ...
  async function addLiquidity(token0Amount: BigNumber, token1Amount: BigNumber) {
    await token0.transfer(pair.address, token0Amount).then((res: any) => res.wait());
    await token1.transfer(pair.address, token1Amount).then((res: any) => res.wait());
    await pair.mint(wallet.address, overrides).then((res: any) => res.wait());
  }
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      await token0.transfer(pair.address, swapAmount).then((res: any) => res.wait());
      await expect(pair.swap(0, expectedOutputAmount.add(1), wallet.address, '0x', overrides)).to.be.revertedWith(
        'UniswapV2: K',
      );
      await pair.swap(0, expectedOutputAmount, wallet.address, '0x', overrides).then((res: any) => res.wait());
    });
  });
  ...
  //   await pair.sync(overrides).then((res: any) => res.wait())
  ...
  //    .then((res: any) => res.wait()) // make the price nice
  ...
  //    .to.eq(initialPrice[0].mul(10).add(newPrice[0].mul(10)))
  //   expect(await pair.price1CumulativeLast())
  //    .to.eq(initialPrice[1].mul(10).add(newPrice[1].mul(10)))
  ...
  it('feeTo:on', async () => {
    ...
    // using 1000 here instead of the symbolic MINIMUM_LIQUIDITY because
    // the amounts only happen to be equal because the initial liquidity amounts were equal
