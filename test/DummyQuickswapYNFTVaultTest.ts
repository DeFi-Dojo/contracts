...
describe("DummyQuickswapYNFTVault", () => {
  ...
  it("should return to deployer on removeVault", async () => {
    ...
    await signers[1].sendTransaction({
      to: dummyQuickswapYnftVault.address,
  ...
  it("should withdraw assets from liquidity pool for all tokens", async () => {
    ...
    await uniswapRouter.addLiquidity.returns([
      MIN_AMOUNT,
      MIN_AMOUNT,
      LIQUIDITY,
    ]);

    await dummyQuickswapYnftVault.createYNFTForEther(
      MIN_AMOUNT,
      MIN_AMOUNT,
      0,
      0,
    ...
    expect(uniswapRouter.removeLiquidity.getCall(0).args[0]).to.equal(
      token0Mock.address
    );
    expect(uniswapRouter.removeLiquidity.getCall(0).args[1]).to.equal(
      token1Mock.address
    );
    expect(uniswapRouter.removeLiquidity.getCall(0).args[2]).to.equal(
      LIQUIDITY
    );
    ...
    expect(uniswapRouter.removeLiquidity.getCall(0).args[5]).to.equal(
      signers[0].address
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(0).args[0]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(0).args[1]).to.equal(0);
    expect(uniswapRouter.swapExactTokensForETH.getCall(0).args[2]).to.eql([
      token0Mock.address,
      WethMock.address,
    ]);
    expect(uniswapRouter.swapExactTokensForETH.getCall(0).args[3]).to.equal(
      dummyQuickswapYnftVault.address
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(1).args[0]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(1).args[1]).to.equal(0);
    expect(uniswapRouter.swapExactTokensForETH.getCall(1).args[2]).to.eql([
      token1Mock.address,
      WethMock.address,
    ]);
    expect(uniswapRouter.swapExactTokensForETH.getCall(1).args[3]).to.equal(
      dummyQuickswapYnftVault.address
    );
  });
  ...
  it("should withdraw assets from liquidity pool for all tokens if WMATIC in pair", async () => {
    ...
    await pairMock.approve.returns(true);
    await stakingRewardsMock.balanceOf.returns(LIQUIDITY);
    await uniswapRouter.addLiquidityETH.returns([
      MIN_AMOUNT,
      MIN_AMOUNT,
      LIQUIDITY,
    ]);
    ...
    expect(uniswapRouter.removeLiquidityETH.getCall(0).args[0]).to.equal(
      token1Mock.address
    );
    expect(uniswapRouter.removeLiquidityETH.getCall(0).args[1]).to.equal(
      LIQUIDITY
    );
    ...
    expect(uniswapRouter.removeLiquidityETH.getCall(0).args[4]).to.equal(
      signers[0].address
    );
  });
  ...
  it("should not withdraw asset from quickswap liquidity pool for already withdrawn token", async () => {
    ...
    await dummyQuickswapYnftVault.withdrawToEther(
      0,
      MIN_AMOUNT,
      MIN_AMOUNT,
      0,
    ...
    expect(uniswapRouter.removeLiquidity.getCall(0).args[3]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.removeLiquidity.getCall(0).args[4]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.removeLiquidity.getCall(0).args[5]).to.equal(
      dummyQuickswapYnftVault.address
    ...
    expect(uniswapRouter.swapExactTokensForETH.getCall(2).args[0]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(2).args[1]).to.equal(0);
    expect(uniswapRouter.swapExactTokensForETH.getCall(2).args[2]).to.eql([
      token0Mock.address,
      WethMock.address,
    ]);
    expect(uniswapRouter.swapExactTokensForETH.getCall(2).args[3]).to.equal(
      dummyQuickswapYnftVault.address
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(3).args[0]).to.equal(
      MIN_AMOUNT
    );
    expect(uniswapRouter.swapExactTokensForETH.getCall(3).args[1]).to.equal(0);
    expect(uniswapRouter.swapExactTokensForETH.getCall(3).args[2]).to.eql([
      token1Mock.address,
      WethMock.address,
    ]);
    expect(uniswapRouter.swapExactTokensForETH.getCall(3).args[3]).to.equal(
      dummyQuickswapYnftVault.address
    );
  });
});
